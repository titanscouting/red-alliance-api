...
exports.submitStrategy = async (db, scouterin, teamin, compin, datain) => {
    ...
    let myobj = {"$set": {scouter: scouterin, competition: compin, team_scouted: teamin, data: datain}};
    let id = await uuidv4()
    try {
        await dbo.collection("data").updateOne({_id: String(id)}, myobj, {upsert:true}).catch(e => {console.error(e);data.err_occur = true;})
    } catch (err) {
        data.err_occur = true
        data.err_reasons.push(err)
        console.error(err)
