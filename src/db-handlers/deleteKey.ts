 ...
 * @desc deletes an API key from the database
...
export default async (db: any, clientID: string, clientKey: string, team: number, scouter: Scouter): Promise<UserReturnData> => {
  ...
  await dbo.collection('api_keys').deleteOne({ _id: clientID }, myobj).catch((e) => { console.error(e); data.err_reasons.push(e); data.err_occur = true; });
  ...
  await dbo2.collection('data').deleteOne({ _id: clientID }, myobj2).catch((e) => { console.error(e); data.err_reasons.push(e); data.err_occur = true; });
  return data;
};
